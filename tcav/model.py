  ...
  """
            ...
            with self.sess.graph.as_default():
                if tf.io.gfile.isdir(model_path):
                    ...
                    if ckpt:
                        ...
                        tf.compat.v1.logging.info(
                            'Loading from SavedModel dir.')
                        tf.saved_model.loader.load(self.sess, ['serve'],
                                                   model_path)
                else:
                    input_graph_def = tf.compat.v1.GraphDef()
                    if model_path.endswith('.pb'):
                        tf.compat.v1.logging.info(
                            'Loading from frozen binary graph.')
                        with tf.io.gfile.GFile(model_path, 'rb') as f:
                            input_graph_def.ParseFromString(f.read())
                    else:
                        tf.compat.v1.logging.info(
                            'Loading from frozen text graph.')
    ...
    """
        return self.adjust_prediction(
            self.sess.run(self.ends['prediction'],
    ...
    """
        tf.compat.v1.logging.warn(
            'label_to_id undefined. Defaults to returning 0.')
        return 0
...
class PublicImageModelWrapper(ImageModelWrapper):
    ...
    def import_graph(saved_path, endpoints, image_value_range, scope='import'):
        ...
        with tf.name_scope(scope) as sc:
            ...
            myendpoints = tf.import_graph_def(graph_def,
                                              graph_inputs,
