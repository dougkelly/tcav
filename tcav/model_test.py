...
class ModelTest_model(ModelWrapper):
    ...
    def __init__(self, model_path=None, node_dict=None):
        super(ModelTest_model, self).__init__(model_path=model_path,
                                              node_dict=node_dict)
...
class ModelTest(googletest.TestCase):
    ...
    def setUp(self):
        ...
        self.tmp_dirs = [
            self.ckpt_dir, self.saved_model_dir, self.frozen_graph_dir
        ]
        ...
        with tf.Session() as sess:
            ...
            tf.io.write_graph(output_graph_def,
                              self.frozen_graph_dir,
                              'graph.pb',
                              as_text=False)
    ...
    def _check_output_and_gradient(self, model_path, import_prefix=False):
        ...
        grad = model.sess.run(model.bottlenecks_gradients['v1'],
                              feed_dict={input_name: 555})
