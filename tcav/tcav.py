  ...
  """
    ...
    """
        ...
        for i in range(len(class_acts)):
            ...
            example = examples[i]
            grad = np.reshape(
                mymodel.get_gradient(act, [class_id], cav.bottleneck, example),
                -1)
            directional_dir_vals.append(np.dot(grad,
                                               cav.get_direction(concept)))
    ...
    """
        ...
        self.relative_tcav = (random_concepts
                              is not None) and (set(concepts)
                                                == set(random_concepts))
        ...
        if run_parallel:
            ...
            for i, res in enumerate(
                    pool.imap(
                        ...
                        self.params), 1):
                tf.compat.v1.logging.info('Finished running param %s of %s' %
                                          (i, len(self.params)))
                results.append(res)
        else:
            for i, param in enumerate(self.params):
                tf.compat.v1.logging.info('Running param %s of %s' %
                                          (i, len(self.params)))
                results.append(
                ...
                tf.compat.v1.logging.info(
                    'Done running %s params. Took %s seconds...' %
    ...
    """
        ...
        def get_random_concept(i):
            return (random_concepts[i]
                    if random_concepts else 'random500_{}'.format(i))
        ...
        for bottleneck in self.bottlenecks:
            for target_in_test, concepts_in_test in self.pairs_to_test:
                for alpha in self.alphas:
                    tf.compat.v1.logging.info('%s %s %s %s', bottleneck,
                                              concepts_in_test, target_in_test,
                                              alpha)
                    params.append(
                        run_params.RunParams(bottleneck, concepts_in_test,
                                             target_in_test,
                                             self.activation_generator,
                                             self.cav_dir, alpha, self.mymodel))
        return params
